...
impl Registry {
    pub fn new() -> Self {
        ...
        let mut handlers: BTreeMap<String, Arc<dyn Provider>> = BTreeMap::new();
        handlers.insert(
            "codex".into(),
            Arc::new(crate::providers::codex::CodexProvider::new()),
        );
        handlers.insert(
            "grok".into(),
...
mod tests {
    ...
    #[test]
    fn claude_5_aliases_route_to_codex() {
        let registry = Registry::new();
        for model in [
            "claude-opus-5",
            "claude-sonnet-5",
    ...
    #[test]
    fn catalog_contains_only_active_providers() {
        let registry = Registry::new();
        ...
        assert!(registry.provider("codex").is_some());
        assert!(registry.provider("grok").is_some());
    }
}
